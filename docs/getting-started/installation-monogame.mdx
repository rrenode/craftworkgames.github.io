...
<Tabs>
  <TabItem value="net-cli" label=".NET Cli" default>
    ```sh
    dotnet add package MonoGame.Extended --version 3.9.0-prerelease.7
    ```
  </TabItem>
  <TabItem value="package-manager" label="Package Manager">
    ```sh
    NuGet\Install-Package MonoGame.Extended -Version 3.9.0-prerelease.7
    ```
    <small>
  ...
  <TabItem value="package-reference" label="Package Reference" default>
    ```xml
    <PackageReference Include="MonoGame.Extended" Version="3.9.0-prerelease.7" />
    ```
    <small>
...
<Tabs>
  <TabItem value="net-cli" label=".NET Cli" default>
    ```sh
    dotnet add package MonoGame.Extended.Content.Pipeline --version 3.9.0-prerelease.7
    ```
  </TabItem>
  <TabItem value="package-manager" label="Package Manager">
    ```sh
    NuGet\Install-Package MonoGame.Extended.Content.Pipeline -Version 3.9.0-prerelease.7
    ```
    <small>
      This command is intended to be used within the Package Manager Console in Visual Studio, as it uses NuGet module's version of [Install-Package](https://docs.microsoft.com/nuget/reference/ps-reference/ps-ref-install-package)
    </small>
  ...
  <TabItem value="package-reference" label="Package Reference" default>
    ```xml
    <PackageReference Include="MonoGame.Extended.Content.Pipeline" Version="3.9.0-prerelease.7" />
    ```
    <small>
        For projects that support [PackageReference](https://docs.microsoft.com/nuget/consume-packages/package-references-in-project-files), copy this XML node into the project file to reference the package. 
    </small>
...
5. Find and add the **MonoGame.Extended.Content.Pipeline.dll** that was downloaded from the NuGet package

:::tip
By default, NuGet will download packages to the global packages directory.  The following shows where the location of the **MonoGame.Extended.Content.Pipeline.dll** will be with the default NuGet configs depending on your operating system.

<Tabs>
  <TabItem value="window" label="Windows" default>
    ```
    %USERPROFILE%\.nuget\packages\monogame.extended.content.pipeline\3.9.0-prerelease.7\tools\MonoGame.Extended.Content.Pipeline.dll
    ```
  </TabItem>
  <TabItem value="mac" label="macOS">
    ```sh
    ~/.nuget/packages/monogame.extended.content.pipeline/3.9.0-prerelease.7/tools/MonoGame.Extended.Content.Pipeline.dll
    ```
  </TabItem>
...
/reference:..\..\..\..\..\..\.nuget\packages\monogame.extended.content.pipeline\3.9.0-prerelease.7\tools\MonoGame.Extended.Content.Pipeline.dll
```
